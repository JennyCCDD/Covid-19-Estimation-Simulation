...
to go
  ...
  if call-for-quarentine
     ...
     if count (turtles with [quarentine? = true]) = 0
            [ask n-of ((accepted-quarentine * number-of-agents) / 100) turtles [set quarentine? call-for-quarentine]]
...
to JGN-network    ;; social network dynamically grows
  ...
  ;;; 
   ...
   let num-links  number-of-agents * (number-of-agents - 1) / 2  ;np=1/2 * N(N-1)
    ...
    [
    ask one-of turtles with [ not quarentine? ]
    [
      if count my-links < z*
      [
  ...
  ;; some new links
  let nm 0
  ask turtles with [ not quarentine? ]
  [
  ...
  ask one-of turtles with [ not quarentine? ]
    ...
    [
     ask one-of turtles ;with [ quarentine? ]
     [
       if (count my-links > 0)
       [
...
to interaction-between-agents   ;; Interaction;interactions between agents, especially between the infected agents and the susceptiple agents
  ask turtles  with [ not quarentine? ] 
    [ move ]
  ask turtles with [infected? and not quarentine? ]
    [ 
      if count my-links > 0
      [
      ask one-of link-neighbors 
        [ 
          ;; interaction 
          if not resistant? and random-float 100 < incubation-rate 
            [ play-exposed ]
         ]     
      ]
    ]
...
to do-virus-checks
  ...
  ask turtles with [exposed? and not quarentine?]
    [
...
to move-virus-status-plot                   
  set-current-plot "move-covid19-spread Status"
  set-current-plot-pen "susceptible"
  plot (count turtles with [not infected? and not resistant? and not quarentine?]) / (count turtles) * 100
  set-current-plot-pen "infected"
  plot (count turtles with [infected? and not quarentine?]) / (count turtles) * 100
  set-current-plot-pen "resistant"
  plot (count turtles with [resistant? and not quarentine?]) / (count turtles) * 100
  set-current-plot-pen "exposed"
  plot (count turtles with [exposed? and not quarentine?])/(count turtles) * 100
end
...
"" ""
PENS
"susceptible" 1.0 0 -15040220 true "" "plot (count turtles with [not infected? and not resistant? ]) / (count turtles) * 100"
"infected" 1.0 0 -5298144 true "" ""
"resistant" 1.0 0 -11053225 true "" ""
"exposed" 1.0 0 -7500403 true "" ""
...
"" ""
PENS
"susceptible" 1.0 0 -13840069 true "" "plot (count turtles with [not infected? and not resistant? and not quarentine?]) / (count turtles) * 100"
"infected" 1.0 0 -2139308 true "" "plot (count turtles with [infected? and not quarentine?]) / (count turtles) * 100"
"resistant" 1.0 0 -7500403 true "" "plot (count turtles with [resistant? and not quarentine?]) / (count turtles) * 100"
"exposed" 1.0 0 -8630108 true "" ""
...
"" ""
PENS
"average degree" 1.0 0 -16777216 true "" "let degree_sum 0\n  ask turtles\n  [\n    set degree_sum degree_sum + count my-links\n   ]\n  plot ( degree_sum / count turtles)"
